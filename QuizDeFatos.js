...
// ____Parte de boas vindas ____

console.log("\n --------QUIZ DE FATOS HISTÓRICOS--------\n\n ");

console.log('Seja Bem - vindo jogador(a)!\n');

// _____Coletando dados do User_________

const Usuario = quiz.question("\n Digite o seu nome: ")

// Embaralhando a coleção de objedos do array => questoes

const questoesEmbaralhadas = questoes.sort(() => Math.random() - 0.5);
//console.log(questoesEmbaralhadas);  -- comentei para que esta linha não fosse executada! 

// selecionando as 10 Questões - já embaralhadas 

var questoesSelecionadas = questoesEmbaralhadas.slice(0, 10);
...
questoesSelecionadas.forEach((pergunta, index) => {
  //utilizamos o método forEach para exibir somente as perguntas embaralhadas e sem resposta
  console.log(`\n ${index + 1}:${pergunta.pergunta}\n`)
  EntradaDaResp = quiz.question(" Resposta: ")

  if (pergunta.resposta == EntradaDaResp) {
    console.log('\nResposta correta!\n');
    TaxaDeAcerto = TaxaDeAcerto + 1
...
else if (TaxaDeAcerto >= 7) {
  console.log("Você acertou a maioria!");
}
else if (TaxaDeAcerto >= 5) {
  console.log("Pratique um pouco mais!");
}
else { console.log("Oh não! tente mais uma vez! "); }


console.log(`\nJogador : ${Usuario}  \nPontuação Final: ${TaxaDeAcerto} Acertos`)
